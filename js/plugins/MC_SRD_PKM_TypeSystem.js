...
* MC's edition
* ==========================================================================
* In MC's edition, state will temporarily change displace the original type
* of a pokemon
* ==========================================================================
* Introduction
* ==========================================================================
...
Imported.MC_SRD_PKM_TypeSystem = true;

MC.PKM.TypeSystem.notetagsLoaded = false;	

MC.PKM.TypeSystem.stab = Number(PluginManager.parameters("MC_SRD_PKM_TypeSystem")["STAB Multiplier"]);

MC.PKM.TypeSystem.x4EffectMessage = String(PluginManager.parameters("MC_SRD_PKM_TypeSystem")["x4 Effectiveness"]);
MC.PKM.TypeSystem.x2EffectMessage = String(PluginManager.parameters("MC_SRD_PKM_TypeSystem")["x2 Effectiveness"]);
MC.PKM.TypeSystem.x05EffectMessage = String(PluginManager.parameters("MC_SRD_PKM_TypeSystem")["x0.5 Effectiveness"]);
MC.PKM.TypeSystem.x025EffectMessage = String(PluginManager.parameters("MC_SRD_PKM_TypeSystem")["x0.25 Effectiveness"]);
MC.PKM.TypeSystem.x0EffectMessage = String(PluginManager.parameters("MC_SRD_PKM_TypeSystem")["No Effect"]);

MC.PKM.TypeSystem.superEffective = [];
MC.PKM.TypeSystem.notVeryEffective = [];
MC.PKM.TypeSystem.noEffect = [];
for(var i = 1; i <= 30; i++) {
   MC.PKM.TypeSystem.superEffective[i] = PluginManager.parameters("MC_SRD_PKM_TypeSystem")["E"+i+" Super Effect"].split(/\s*,\s*/i);
   MC.PKM.TypeSystem.notVeryEffective[i] = PluginManager.parameters("MC_SRD_PKM_TypeSystem")["E"+i+" NotVery Effect"].split(/\s*,\s*/i);
   MC.PKM.TypeSystem.noEffect[i] = PluginManager.parameters("MC_SRD_PKM_TypeSystem")["E"+i+" No Effect"].split(/\s*,\s*/i);
}

MC.PKM.TypeSystem._DataManager_isDatabaseLoaded = DataManager.isDatabaseLoaded;
DataManager.isDatabaseLoaded = function() {
   if(!MC.PKM.TypeSystem._DataManager_isDatabaseLoaded.call(this)) return false;
   if(!MC.PKM.TypeSystem.notetagsLoaded) {
	   ...
	   MC.PKM.TypeSystem.notetagsLoaded = true;
   }
   return true;
};
...
Game_Actor.prototype.getPKMTypes = function() {
    ...
    var result = [];
    //* the type system in 'state' will now temporarily displace the current type of the PKM
    //* if the type of a state is defined
    ...
    var actor_class = $dataClasses[this.actor().classId];
    if(this.actor().PKMElements) result = this.actor().PKMElements;
    
    if(actor_class.PKMElements){
        for(var i = 0; i < actor_class.length; ++i){
            if(!result.contains(actor_class[i])) {
                result.push(actor_class[i]);
            }
        }
        result.sort();
    }
...
MC.PKM.TypeSystem._Game_Action_apply = Game_Action.prototype.apply;
Game_Action.prototype.apply = function(target) {
   MC.PKM.TypeSystem._Game_Action_apply.call(this, target);
   target.result().isDamage = (this.item().damage.hitType > 0);
};
...
Game_Action.prototype.calcPKMElementRate = function(target) {
	...
	if(defElements.length > 0 && atkElement > 0) {
		for(var i = 0; i < MC.PKM.TypeSystem.superEffective[atkElement].length; i++) {
			if(defElements.contains(String(MC.PKM.TypeSystem.superEffective[atkElement][i]))) {
				effectiveness += 1;
			}
		}

		for(var i = 0; i < MC.PKM.TypeSystem.notVeryEffective[atkElement].length; i++) {
			if(defElements.contains(String(MC.PKM.TypeSystem.notVeryEffective[atkElement][i]))) {
				effectiveness -= 1;
			}
		}

		for(var i = 0; i < MC.PKM.TypeSystem.noEffect[atkElement].length; i++) {
			if(defElements.contains(String(MC.PKM.TypeSystem.noEffect[atkElement][i]))) {
				result = 0;
			}
		}
	}
	...
	if(user.PKMElements && user.PKMElements.contains(String(atkElement))) {
		result *= MC.PKM.TypeSystem.stab;
	}

	return result;
}

MC.PKM.TypeSystem._Window_BattleLog_displayCritical = Window_BattleLog.prototype.displayCritical;
Window_BattleLog.prototype.displayCritical = function(target) {
   if(target.result().isDamage) {
	   if(target.result().moveEffectiveness >= 4 && MC.PKM.TypeSystem.x4EffectMessage.length > 0) {
		   this.push('addText', MC.PKM.TypeSystem.x4EffectMessage);
	   } else if(target.result().moveEffectiveness === 2 && MC.PKM.TypeSystem.x2EffectMessage.length > 0) {
		   this.push('addText', MC.PKM.TypeSystem.x2EffectMessage);
	   } else if(target.result().moveEffectiveness === 0.5 && MC.PKM.TypeSystem.x05EffectMessage.length > 0) {
		   this.push('addText', MC.PKM.TypeSystem.x05EffectMessage);
	   } else if(target.result().moveEffectiveness <= 0.25 && target.result().moveEffectiveness > 0 && MC.PKM.TypeSystem.x025EffectMessage.length > 0) {
		   this.push('addText', MC.PKM.TypeSystem.x025EffectMessage);
	   } else if(target.result().moveEffectiveness === 0 && MC.PKM.TypeSystem.x0EffectMessage.length > 0) {
		   this.push('addText', MC.PKM.TypeSystem.x0EffectMessage);
	   }
   }

   MC.PKM.TypeSystem._Window_BattleLog_displayCritical.call(this, target);
};