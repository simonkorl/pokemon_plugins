 ...
 * using the MC_SRD_PKM_TypeSystem.
 *
...
Imported.MC_SRD_PKM_TypeDisplay = true;

MC.PKM.TypeDisplay.noElementText = String(PluginManager.parameters('MC_SRD_PKM_TypeDisplay')['No Element Text']);

MC.PKM.TypeDisplay.elementColors = [];
MC.PKM.TypeDisplay.elementColors[0] = 0;
for(var i = 1; i <= 30; i++) {
	MC.PKM.TypeDisplay.elementColors[i] = Number(PluginManager.parameters('MC_SRD_PKM_TypeDisplay')['Element ' + i + ' Color']);
}

if(Imported.MC_SRD_PKM_TypeSystem) {

	Window_Base.prototype.drawActorClass = function(actor, x, y, width) {
        width = width || 168;
        var types;
	    if(types = actor.getPKMTypes()) {
	    	//var types = actor.actor().PKMElements;
	    	this.changeTextColor(this.textColor(MC.PKM.TypeDisplay.elementColors[types[0]]));
		    ...
		    if(types[1]) {
		    	...
		    	this.changeTextColor(this.textColor(MC.PKM.TypeDisplay.elementColors[types[1]]));
		    	this.drawText($dataSystem.elements[types[1]], x + txtW, y, width / 2, 'left');
		    }
	    } else {
	    	this.resetTextColor();
		    this.drawText(MC.PKM.TypeDisplay.noElementText, x, y, width, 'left');
	    } 
	};

} else {
	alert("Hello! There is an error!");
	alert("Currently, you have MC_SRD_PKM_TypeDisplay installed; this Plugin is an extension Plugin to MC_SRD_PKM_TypeSystem.");
	alert("Either you don't have MC_SRD_PKM_TypeSystem, or you don't have the latest version of it.");
	window.close();
}