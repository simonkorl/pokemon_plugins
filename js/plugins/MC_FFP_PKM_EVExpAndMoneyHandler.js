 ...
 *
 * @help
 * This plugin is an add-on to MC_FFP_PKM_PokemonBaseStats.js.
...
Imported.MC_FFP_PKM_EVExpAndMoneyHandler = true;

var MC = MC || {};
MC.BattleEndHandler = MC.BattleEndHandler || {};

MC.BattleEndHandler.parameters = PluginManager.parameters('MC_FFP_PKM_EVExpAndMoneyHandler');
...
Game_Enemy.prototype.exp = function() {
	...
	if (Imported.FFP_EnemyLevels) {
		level = this.level();
	} else if (eval(MC.BattleEndHandler.parameters['AverageLevel'])) {
	...
	new_exp = parseInt(eval(String(MC.BattleEndHandler.parameters['ExpFormula']))); //((a*b*l)/7);
...
Game_Enemy.prototype.gold = function() {
	...
	if (Imported.FFP_EnemyLevels) {
		level = this.level();
	} else if (Boolean(MC.BattleEndHandler.parameters['AverageLevel'])) {
		level = $gameParty.averageLevel();
	};
	...
	newGoldValue = parseInt(eval(String(MC.BattleEndHandler.parameters['MoneyFormula'])));
